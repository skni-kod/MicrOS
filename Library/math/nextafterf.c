...
float nextafterf(float x, float y)
{
    ...
    if((y > x && x >= 0) || (y < x && x <= 0))
    {
        ...
        if(y > x && x >= 0)
        {
            dbyte_given.f = copysignf(dbyte_given.f, 1.0);
        }
        if(y < x && x <= 0)
        {
            dbyte_given.f = copysignf(dbyte_given.f, -1.0);
        ...
        // Handle carry if exist through from 1 to 1 byte
        for(int i = 1 ; i < 2; i++)
        {
        ...
        if(carry == 1)
        {
            ...
            // If it's lower than 127 add carry
            ...
            else
            {
                ...
                else
                {
                    ...
                    dbyte_given.bytes[2] &= 0x7F;
                    // Increase last bytes
                    unsigned char last_7_bytes_of_exponent = dbyte_given.bytes[3] & 0x7F;
