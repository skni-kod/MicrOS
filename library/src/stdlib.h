...
/*
    ...
    \param C-string beginning with the representation of an integral number.
...
/*
    ...
    \param Array in memory where to store the resulting null-terminated string.
    \param Numerical base used to represent the value as a string, between 2 and 36, where 10 means decimal base, 16 hexadecimal, 8 octal, and 2 binary.
...
/*
    ...
    \param Reference to an already allocated object of type char*, whose value is set by the function to the next character in str after the numerical value. This parameter can also be a null pointer, in which case it is not used.
...
/*
    ...
    \param Size of the memory block, in bytes. size_t is an unsigned integral type.
...
/*
    ...
    \param Size of the memory block, in bytes. size_t is an unsigned integral type.
    \param Alignment of the block (eg. if align is set to 0x1000, then block will be allocated on addresses like 0x01110000, 0x01111000, 0x01112000,...).
    \return On success, a pointer to the memory block allocated by the function. If the function failed to allocate the requested block of memory, a null pointer is returned.
 */
...
/*
    ...
    \param Pointer to a memory block previously allocated with malloc, calloc or realloc.
    \param New size for the memory block, in bytes. size_t is an unsigned integral type.
...
/*
    ...
    \param An integer value to be used as seed by the pseudo-random number generator algorithm.
 */
