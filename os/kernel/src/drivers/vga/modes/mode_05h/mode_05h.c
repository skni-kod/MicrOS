...
int8_t mode05h_set_mode()
{
    ...
    mode05h_clear_screen();
    video_card_set_turn_on_buffer_func(&mode05h_turn_on_buffer);
    video_card_set_turn_off_buffer_func(&mode05h_turn_off_buffer);
    video_card_set_is_buffer_on_func(&mode05h_is_buffer_on);
    video_card_set_swap_buffers_func(&mode05h_swap_buffers);
    ...
    video_card_set_swap_external_buffer(&mode05h_swap_external_buffer);
...
int8_t mode05h_turn_on_buffer()
{
    ...
    video_card_set_draw_pixel_func(&mode05h_draw_pixel_buffered);
    video_card_set_draw_line_func(&mode05h_draw_line_buffered);
...
int8_t mode05h_draw_line(uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by)
{
    ...
    if(_abs(dx) >= _abs(dy))
    {
        ...
        if(ax > bx)
            for(int x = bx; x <= ax; ++x)
                mode05h_draw_pixel(color, x, a * x + b);
        else
            for(int x = ax; x <= bx; ++x)
                mode05h_draw_pixel(color, x, a * x + b);
    }
    else
    {
        ...
        if(ay > by)
            for(int y = by; y <= ay; ++ y)
                mode05h_draw_pixel(color, a * y + b, y);
        else
            for(int y = ay; y <= by; ++ y)
                mode05h_draw_pixel(color, a * y + b, y);
    }
...
int8_t mode05h_draw_line_buffered(uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by)
{
    ...
    if(_abs(dx) >= _abs(dy))
    {
        ...
        if(ax > bx)
            for(int x = bx; x <= ax; ++x)
                mode05h_draw_pixel_buffered(color, x, a * x + b);
        else
            for(int x = ax; x <= bx; ++x)
                mode05h_draw_pixel_buffered(color, x, a * x + b);
    }
    else
    {
        ...
        if(ay > by)
            for(int y = by; y <= ay; ++ y)
                mode05h_draw_pixel_buffered(color, a * y + b, y);
        else
            for(int y = ay; y <= by; ++ y)
                mode05h_draw_pixel_buffered(color, a * y + b, y);
    }
...
int8_t mode05h_draw_pixel_external_buffer(uint8_t* buffer, uint16_t mode, int8_t color, uint16_t x, uint16_t y){
    ...
    unsigned int offset = (y/2 * MODE05H_WIDTH + x)/4;
	unsigned bit_no = x % 4;
    bit_write(*(buffer + (y%2) * MODE05H_HEIGHT * MODE05H_WIDTH / 8 + offset), (1<<(7 - (2 * bit_no))), (color & 0x2));
    bit_write(*(buffer + (y%2) * MODE05H_HEIGHT * MODE05H_WIDTH / 8 + offset), (1<<(7 - (2 * bit_no+1))), (color & 0x1));
    return 0;
}
int8_t mode05h_draw_line_external_buffer(uint8_t* buffer, uint16_t mode, uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by){
        ...
        if(_abs(dx) >= _abs(dy))
        {
            ...
            if(ax > bx)
                for(int x = bx; x <= ax; ++x)
                    mode05h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
            else
                for(int x = ax; x <= bx; ++x)
                    mode05h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
        }
        else
        {
            ...
            if(ay > by)
                for(int y = by; y <= ay; ++ y)
                    mode05h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
            else
                for(int y = ay; y <= by; ++ y)
                    mode05h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
        }
...
int8_t mode05h_swap_external_buffer(uint8_t* buffer, uint16_t mode){
    memcpy(VGA_VRAM_2, buffer, MODE05H_WIDTH * MODE05H_HEIGHT / 8);
    memcpy(VGA_VRAM_2 + 0x2000, buffer + MODE05H_WIDTH * MODE05H_HEIGHT / 8, MODE05H_WIDTH * MODE05H_HEIGHT / 8);
    //memcpy(VGA_VRAM, mode13h_buffer, MODE13H_HEIGHT * MODE13H_WIDTH);
    return 0;
}
uint8_t* mode05h_create_external_buffer(uint16_t mode){
    uint8_t* ptr =  heap_kernel_alloc(2 * MODE05H_HEIGHT * MODE05H_WIDTH / 8, 0);
    memset(ptr, 0, 2 * MODE05H_HEIGHT * MODE05H_WIDTH / 8);
    return ptr;
}