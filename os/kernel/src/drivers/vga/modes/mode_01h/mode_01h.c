...
int8_t mode01h_swap_external_buffer(uint8_t* buffer, uint16_t mode){
    memcpy((screen*)VGA_MODE_01H_BASE_ADDR, (screen*)buffer, MODE01H_HEIGHT * MODE01H_WIDTH * sizeof(screen));
    return 0;
}
uint8_t* mode01h_create_external_buffer(uint16_t mode){
	return heap_kernel_alloc(MODE01H_HEIGHT * MODE01H_WIDTH * sizeof(screen), 0);
}