...
int8_t mode06h_set_mode()
{
    ...
    video_card_set_swap_external_buffer(&mode06h_swap_external_buffer);
...
int8_t mode06h_draw_pixel_external_buffer(uint8_t* buffer, uint16_t mode, int8_t color, uint16_t x, uint16_t y){
    ...
    unsigned int offset = (y/2 * MODE06H_WIDTH + x)/8;
	unsigned bit_no = x % 8;
    bit_write(*(buffer + (y%2) * MODE06H_HEIGHT * MODE06H_WIDTH / 8 + offset), (1<<(7 - (2 * bit_no))), (color & 0x1));
    return 0;
}
int8_t mode06h_draw_line_external_buffer(uint8_t* buffer, uint16_t mode, uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by){
        ...
        if(_abs(dx) >= _abs(dy))
        {
            ...
            if(ax > bx)
                for(int x = bx; x <= ax; ++x)
                    mode06h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
            else
                for(int x = ax; x <= bx; ++x)
                    mode06h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
        }
        else
        {
            ...
            if(ay > by)
                for(int y = by; y <= ay; ++ y)
                    mode06h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
            else
                for(int y = ay; y <= by; ++ y)
                    mode06h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
        }
...
int8_t mode06h_swap_external_buffer(uint8_t* buffer, uint16_t mode){
    memcpy(VGA_VRAM_2, buffer, MODE06H_WIDTH * MODE06H_HEIGHT / 16);
    memcpy(VGA_VRAM_2 + 0x2000, buffer + MODE06H_WIDTH * MODE06H_HEIGHT /16, MODE06H_WIDTH * MODE06H_HEIGHT / 16);
    //memcpy(VGA_VRAM, mode13h_buffer, MODE13H_HEIGHT * MODE13H_WIDTH);
    return 0;
}
uint8_t* mode06h_create_external_buffer(uint16_t mode){
    uint8_t* ptr = heap_kernel_alloc(2 * MODE06H_HEIGHT * MODE06H_WIDTH / 16, 0);
    memset(ptr, 0, 2 * MODE06H_HEIGHT * MODE06H_WIDTH / 16);
    return ptr;
}