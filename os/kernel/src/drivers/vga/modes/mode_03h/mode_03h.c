...
int8_t mode03h_swap_external_buffer(uint8_t* buffer, uint16_t mode){
    memcpy((screen*)VGA_MODE_03H_BASE_ADDR, (screen*)buffer, MODE03H_HEIGHT * MODE03H_WIDTH * sizeof(screen));
    return 0;
}
uint8_t* mode03h_create_external_buffer(uint16_t mode){
	return heap_kernel_alloc(MODE03H_HEIGHT * MODE03H_WIDTH * sizeof(screen), 0);
}