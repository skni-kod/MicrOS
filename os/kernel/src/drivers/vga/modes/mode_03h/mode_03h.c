...
int8_t mode03h_set_mode()
{
	...
	video_card_set_turn_on_buffer_func(mode03h_turn_on_buffer);
    video_card_set_turn_off_buffer_func(mode03h_turn_off_buffer);
    video_card_set_is_buffer_on_func(mode03h_is_buffer_on);
    video_card_set_swap_buffers_func(mode03h_swap_buffers);
	video_card_set_print_char_func(mode03h_print_char);
	video_card_set_print_char_color_func(mode03h_print_char_color);
	video_card_set_print_string_func(mode03h_print_string);
	video_card_set_print_string_color_func(mode03h_print_string_color);
	video_card_set_set_char_func(mode03h_set_char);
	video_card_set_get_char_func(mode03h_get_char);
	video_card_set_set_color_func(mode03h_set_color);
	video_card_set_get_color_func(mode03h_get_color);
	video_card_set_set_char_and_color_func(mode03h_set_char_and_color);
	video_card_set_get_char_and_color_func(mode03h_get_char_and_color);
	video_card_set_set_cursor_pos_func(mode03h_set_cursor_pos);
	video_card_set_get_cursor_pos_func(mode03h_get_cursor_pos);
	video_card_set_turn_cursor_on_func(mode03h_turn_cursor_on);
	video_card_set_turn_cursor_off_func(mode03h_turn_cursor_off);
    video_card_set_draw_pixel_func(mode03h_draw_pixel);
    video_card_set_draw_line_func(mode03h_draw_line);
    video_card_set_draw_circle_func(mode03h_draw_circle);
    video_card_set_draw_rectangle_func(mode03h_draw_rectangle);
    video_card_set_clear_screen_func(mode03h_clear_screen);

	video_card_set_print_char_external_buffer(mode03h_print_char_external_buffer);
	video_card_set_print_char_color_external_buffer(mode03h_print_char_color_external_buffer);
	video_card_set_print_string_external_buffer(mode03h_print_string_external_buffer);
	video_card_set_print_string_color_external_buffer(mode03h_print_string_color_external_buffer);
	video_card_set_set_char_external_buffer(mode03h_set_char_external_buffer);
	video_card_set_get_char_external_buffer(mode03h_get_char_external_buffer);
	video_card_set_set_color_external_buffer(mode03h_set_color_external_buffer);
	video_card_set_get_color_external_buffer(mode03h_get_color_external_buffer);
	video_card_set_set_char_and_color_external_buffer(mode03h_set_char_and_color_external_buffer);
	video_card_set_get_char_and_color_external_buffer(mode03h_get_char_and_color_external_buffer);
    return 0x03;
}
...
int8_t mode03h_turn_on_buffer()
{
	...
	video_card_set_print_string_color_func(mode03h_print_string_color_buffered);
	video_card_set_set_char_func(mode03h_set_char_buffered);
	video_card_set_get_char_func(mode03h_get_char_buffered);
	video_card_set_set_color_func(mode03h_set_color_buffered);
	video_card_set_get_color_func(mode03h_get_color_buffered);
	video_card_set_set_char_and_color_func(mode03h_set_char_and_color_buffered);
	video_card_set_get_char_and_color_func(mode03h_get_char_and_color_buffered);
	video_card_set_set_cursor_pos_func(mode03h_set_cursor_pos_buffered);
	video_card_set_get_cursor_pos_func(mode03h_get_cursor_pos_buffered);
    video_card_set_draw_pixel_func(mode03h_draw_pixel_buffered);
    video_card_set_draw_line_func(mode03h_draw_line_buffered);
    video_card_set_draw_circle_func(mode03h_draw_circle_buffered);
    video_card_set_draw_rectangle_func(mode03h_draw_rectangle_buffered);
    video_card_set_clear_screen_func(mode03h_clear_screen_buffered);
    return 0;
}
...
{
    if(mode03h_buffer == NULL) return -1;
    memcpy((screen_char*)VGA_MODE_03H_BASE_ADDR, mode03h_buffer, MODE03H_HEIGHT * MODE03H_WIDTH * sizeof(screen_char));
	__vga_update_cursor(mode03h_buffer_cursor_x, mode03h_buffer_cursor_y);
    return 0;
}
...
int8_t mode03h_draw_line(uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by)
{
    ...
    if(_abs(dx) >= _abs(dy))
    {
        ...
        if(ax > bx)
            for(int x = bx; x <= ax; ++x)
                mode03h_draw_pixel(color, x, a * x + b);
        else
            for(int x = ax; x <= bx; ++x)
                mode03h_draw_pixel(color, x, a * x + b);
    }
    else
    {
        ...
        if(ay > by)
            for(int y = by; y <= ay; ++ y)
                mode03h_draw_pixel(color, a * y + b, y);
        else
            for(int y = ay; y <= by; ++ y)
                mode03h_draw_pixel(color, a * y + b, y);
    }
...
int8_t mode03h_print_char_buffered(char character)
{
	...
	{
		__mode03h_print_char_buffer((uint16_t*)mode03h_buffer, 0, &mode03h_buffer_cursor_x, &mode03h_buffer_cursor_y, character);
		return 0;
	}
}
...
int8_t mode03h_print_char_color_buffered(char character, uint8_t color)
{
	...
	{
		__mode03h_print_char_color_buffer((uint16_t*)mode03h_buffer, 0, &mode03h_buffer_cursor_x, &mode03h_buffer_cursor_y, character, color);
		return 0;
	}
}
...
int8_t mode03h_print_string_buffered(const char* string)
{
	...
	{
		__mode03h_print_string_buffer((uint16_t*)mode03h_buffer, 0, &mode03h_buffer_cursor_x, &mode03h_buffer_cursor_y, string);
		return 0;
	}
}
...
int8_t mode03h_print_string_color_buffered(const char* string, uint8_t color)
{
	...
	{
		__mode03h_print_string_color_buffer((uint16_t*)mode03h_buffer, 0, &mode03h_buffer_cursor_x, &mode03h_buffer_cursor_y, string, color);
		return 0;
	}
}
int8_t mode03h_set_char_buffered(uint16_t x, uint16_t y, char character)
{
	...
	{
		__mode03h_set_char_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, character);
		return 0;
	}
}
...
int8_t mode03h_get_char_buffered(uint16_t x, uint16_t y, char* character)
{
	...
	{
		__mode03h_get_char_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, character);
		return 0;
	}
}
...
int8_t mode03h_set_color_buffered(uint16_t x, uint16_t y, uint8_t color)
{
	...
	{
		__mode03h_set_color_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, color);
		return 0;
	}
}
...
int8_t mode03h_get_color_buffered(uint16_t x, uint16_t y, uint8_t* color)
{
	...
	{
		__mode03h_get_color_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, color);
		return 0;
	}
}
...
int8_t mode03h_set_char_and_color_buffered(uint16_t x, uint16_t y, char character, uint8_t color)
{
	...
	{
		__mode03h_set_char_and_color_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, character, color);
		return 0;
	}
}
...
int8_t mode03h_get_char_and_color_buffered(uint16_t x, uint16_t y, char* character, uint8_t* color)
{
	...
	{
		__mode03h_get_char_and_color_buffer((uint16_t*)mode03h_buffer, 0, mode03h_buffer_cursor_x, mode03h_buffer_cursor_y, character, color);
		return 0;
	}
}
...
int8_t mode03h_draw_pixel_buffered(uint8_t color, uint16_t x, uint16_t y)
{
	...
	mode03h_buffer[__vga_calcualte_position_without_offset(x, y)] = s;
	return 0;
}
...
int8_t mode03h_clear_screen_buffered()
{
	...
	for (uint16_t i = 0; i < VGA_MODE_02H_SCREEN_ROWS; ++i)
    {
        ...
        for (uint16_t j = 0; j < VGA_MODE_02H_SCREEN_COLUMNS; ++j)
        {
            ...
            // Clear
            mode03h_buffer[pos].character.ascii_code = 0;
            mode03h_buffer[pos].character.color.color_without_blink = col;
        }
    }
	return 0;
}
...
int8_t mode03h_draw_line_external_buffer(uint8_t* buffer, uint16_t mode, uint8_t color, uint16_t ax, uint16_t ay, uint16_t bx, uint16_t by)
{
    ...
    if(_abs(dx) >= _abs(dy))
    {
        ...
        if(ax > bx)
            for(int x = bx; x <= ax; ++x)
                mode03h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
        else
            for(int x = ax; x <= bx; ++x)
                mode03h_draw_pixel_external_buffer(buffer, mode, color, x, a * x + b);
    }
    else
    {
        ...
        if(ay > by)
            for(int y = by; y <= ay; ++ y)
                mode03h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
        else
            for(int y = ay; y <= by; ++ y)
                mode03h_draw_pixel_external_buffer(buffer, mode, color, a * y + b, y);
    }
...
int8_t mode03h_swap_external_buffer(uint8_t* buffer, uint16_t mode){
    memcpy((screen_char*)VGA_MODE_03H_BASE_ADDR, (screen_char*)buffer, MODE03H_HEIGHT * MODE03H_WIDTH * sizeof(screen_char));
    return 0;
}
...
int8_t __mode03h_print_char_color_buffer(uint16_t* buffer, uint16_t mode, uint16_t* x, uint16_t* y, char character, uint8_t color)
{
	...
	screen_char* ptr = (screen_char*) buffer;
    if (character != '\n')
    {
        ptr[pos].character.ascii_code = character;
		if (color != 0)
        {
			vga_color c;
			c.color_without_blink.background = color >> 4;
			c.color_without_blink.letter = color && 15;
            ptr[pos].character.color = c;
        }
        *x += 1;
        if (*x == MODE03H_WIDTH)
        {
...
int8_t __mode03h_print_string_buffer(uint16_t* buffer, uint16_t mode, uint16_t* x, uint16_t* y, const char* string)
{
    ...
    while (*ptr != 0)
    {
        __mode03h_print_char_buffer(buffer, mode, x, y, *ptr);
        ptr++;
    }
	return 0;
}
...
int8_t __mode03h_print_string_color_buffer(uint16_t* buffer, uint16_t mode, uint16_t* x, uint16_t* y, const char* string, uint8_t color)
{
    ...
    while (*ptr != 0)
    {
        __mode03h_print_char_color_buffer(buffer, mode, x, y, *ptr, color);
        ptr++;
    }
	return 0;
}
...
void __mode03h_newline(uint16_t* buffer, uint16_t* x, uint16_t* y)
{
	*x = 0;
    ...
    if (*y == MODE03H_HEIGHT)
    {
        ...
        for (uint16_t i = 0; i < MODE03H_HEIGHT - 1; ++i)
        {
            for (uint16_t j = 0; j < MODE03H_WIDTH; ++j)
            {
                uint16_t pos = __mode03h_calcualte_position(j, i);
                // Copy byte from next line
                buffer[pos] = buffer[pos + MODE03H_WIDTH];
            }
        }
        *y = MODE03H_HEIGHT - 1;
