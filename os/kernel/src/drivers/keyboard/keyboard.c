...
void keyboard_put_key_to_buffer(unsigned char scancode, unsigned char ascii)
{
    ...
    volatile keyboard_scan_ascii_pair *character = (keyboard_scan_ascii_pair *)((DMA_ADDRESS_OFFSET + (0x0040 * 0x10)) + (*buffer_write));
...
bool keyboard_get_key_from_buffer(keyboard_scan_ascii_pair *scan_ascii_pair)
{
    ...
    keyboard_scan_ascii_pair *tmp = (keyboard_scan_ascii_pair *)((DMA_ADDRESS_OFFSET + (0x0040 * 0x10)) + (*buffer_read));
