...
void toon_vertex(shader* _shader, vec4 vertex, vec3 norm, vec2f uv, int vertnum)
{
...
color light_col_d2 = {200,200,200,255};
color spec_col = {255,255,255,255};
...
color toon_fragment(shader* _shader)
{
    ...
    float diffuse = dot_product_vec3(n, light_dir) > 0.0f ? 1.0f : 0.0f;
    color diff_c = mul_color_f(light_col_d2, diffuse);


    color c = add_color(ambient, diff_c);
    ...
    float spec = pow(MAX(dot_product_vec3(n, halfway), 0.0f), 32.0f) > 0.0f ? 1.0f : 0.0f;

    color sc = mul_color_f(spec_col, spec);
    ...
    // float spec = pow(MAX(dot_product_vec3(n, halfway), 0.0f), 128.0f);
