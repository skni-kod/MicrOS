...
void writeRegisters(uint8_t* registers)
{
    ...
    {
        (void)inb(attributeControllerResetPort);
...
void writeRegistersText(uint8_t* registers)
{
    ...
    outb(crtcIndexPortText, 0x03);
    outb(crtcDataPortText, inb(crtcDataPortText) | 0x80);
    outb(crtcIndexPortText, 0x11);
    outb(crtcDataPortText, inb(crtcDataPortText) & ~0x80);
...
static unsigned get_fb_seg(void)
{
	...
	outb(graphicsControllerIndexPort, 6);
	seg = inb(graphicsControllerDataPort);
	seg >>= 2;
	seg &= 3;
	switch(seg)
	{
...
static void set_plane(unsigned p)
{
	...
	pmask = 1 << p;
/* set read plane */
	outb(graphicsControllerIndexPort, 4);
	outb(graphicsControllerDataPort, p);
/* set write plane */
	outb(sequencerIndexPort, 2);
	outb(sequencerDataPort, pmask);
}
...
set_plane() modifies GC 4 and SEQ 2, so save them as well */
	outb(sequencerIndexPort, 2);
	seq2 = inb(sequencerDataPort);

	outb(sequencerIndexPort, 4);
	seq4 = inb(sequencerDataPort);
/* turn off even-odd addressing (set flat addressing)
assume: chain-4 addressing already off */
	outb(sequencerDataPort, seq4 | 0x04);

	outb(graphicsControllerIndexPort, 4);
	gc4 = inb(graphicsControllerDataPort);

	outb(graphicsControllerIndexPort, 5);
	gc5 = inb(graphicsControllerDataPort);
/* turn off even-odd addressing */
	outb(graphicsControllerDataPort, gc5 & ~0x10);

	outb(graphicsControllerIndexPort, 6);
	gc6 = inb(graphicsControllerDataPort);
/* turn off even-odd addressing */
	outb(graphicsControllerDataPort, gc6 & ~0x02);
...
/* restore registers */
	outb(sequencerIndexPort, 2);
	outb(sequencerDataPort, seq2);
	outb(sequencerIndexPort, 4);
	outb(sequencerDataPort, seq4);
	outb(graphicsControllerIndexPort, 4);
	outb(graphicsControllerDataPort, gc4);
	outb(graphicsControllerIndexPort, 5);
	outb(graphicsControllerDataPort, gc5);
	outb(graphicsControllerIndexPort, 6);
